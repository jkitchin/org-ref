...
org-ref can display rendered equation images in tooltips when you hover over equation references (like =eqref:eq:1=). This feature requires that you have already generated preview images for your equations using org-mode's =org-latex-preview= (=C-c C-x C-l=).
...
When enabled:
- Hovering over an equation reference will show the rendered equation image in a tooltip (GUI Emacs only)
- If no preview exists, shows the LaTeX code (default behavior)
