...
#+BEGIN_SRC emacs-lisp
(define-key org-mode-map (kbd "C-c ]") 'org-ref-insert-link-menu)
#+END_SRC
...
** Equation image tooltips

org-ref can display rendered equation images in the minibuffer when you hover over equation references (like =eqref:eq:1=). This feature requires that you have already generated preview images for your equations using org-mode's =org-latex-preview= (=C-c C-x C-l=).
...
When enabled:
- Hovering over an equation reference will show the rendered equation image in the minibuffer (GUI Emacs only)
- If no preview exists, shows the LaTeX code in a tooltip (default behavior)
- Automatically falls back to text display in terminal Emacs
- Works with all reference types (=eqref=, =ref=, =autoref=, =cref=, etc.)
- Supports both =\label{...}= and =#+name:= style labels
- Uses caching for good performance

This feature is compatible with org-mode's built-in =org-latex-preview= and also works with the =math-preview= package.
